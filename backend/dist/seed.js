...
async function seed() {
    ...
    for (const k of finansijskiPartneri) {
        await AppDataSource.query(`INSERT INTO kompanija (naziv, websajt, kontakt, tip) VALUES ($1, $2, $3, $4)`, [k.naziv, k.websajt, k.kontakt, 'finansijski']);
        console.log(`  [finansijski] ${k.naziv}`);
    }
    for (const k of robniPartneri) {
        await AppDataSource.query(`INSERT INTO kompanija (naziv, websajt, kontakt, tip) VALUES ($1, $2, $3, $4)`, [k.naziv, k.websajt, k.kontakt, 'robni']);
    ...
    for (const u of users) {
        const exists = await AppDataSource.query(`SELECT id FROM korisnik WHERE username = $1`, [u.username]);
        if (exists.length > 0) {
            await AppDataSource.query(`UPDATE korisnik SET lozinka=$1, ime=$2, prezime=$3, tip=$4 WHERE username=$5`, [u.lozinka, u.ime, u.prezime, u.tip, u.username]);
            console.log(`Ažuriran korisnik: ${u.username}`);
        }
        else {
            await AppDataSource.query(`INSERT INTO korisnik (username, lozinka, ime, prezime, tip) VALUES ($1,$2,$3,$4,$5)`, [u.username, u.lozinka, u.ime, u.prezime, u.tip]);
